	...
	if (isMDMin) {
		return (
			<Dialog open={open} onOpenChange={(e) => openDrawer(e)}>
				...
				<DialogContent
					dir={lang == "en" ? "ltr" : "rtl"}
					className="max-w-106.25 min-w-80 sm:max-w-fit w-fit bg-secondary backdrop-filter p-3 gap-y-3 backdrop-blur-md rounded-2xl"
				>
					<DialogHeader>
							...
							<DialogTitle>
								{formType != "Info"
									? formType != "Edit"
										...
										: t.edit
									: t.info}
								{formType != "Edit" ? translateTitle() : drawerTitle}
							</DialogTitle>
						</div>
					</DialogHeader>
					{formType == "Info" ? (
