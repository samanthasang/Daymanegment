...
function useShareList() {
  ...
  const [listAfterFilter, setListAfterFilter] = useState< TShare[] | undefined>(ListShare)
  ...
  const filterdList = () => {
    ...
    if (hasdateTo && hasdateFrom && fromDay ) {
      if (toDay) {
          filterArrayDay =  ListShare.filter((list) => list.date >= fromDay && list.date <= toDay)
      }
    }
    ...
    if (fromDay && toDay && fromDay != toDay) {
      filterArrayDay = ListShare.filter((list) => Math.floor(+list.date).toString() >= fromDay &&
