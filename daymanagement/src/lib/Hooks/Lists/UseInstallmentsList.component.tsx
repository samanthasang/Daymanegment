...
function useInstallmentsList() {
  ...
  const [listAfterFilter, setListAfterFilter] = useState< TInstallmentst[] | undefined>(ListInstallmentst)
  ...
  const filterdList = () => {
    ...
    if (hasdateTo && hasdateFrom && fromDay ) {
      if (toDay) {
          filterArrayDay =  ListInstallmentst.filter((list) => list.lastUpdate >= fromDay && list.lastUpdate <= toDay)
      }
    }
    ...
    if (fromDay && toDay && fromDay != toDay) {
      filterArrayDay = ListInstallmentst.filter((list) => Math.floor(+list.lastUpdate).toString() >= fromDay &&
